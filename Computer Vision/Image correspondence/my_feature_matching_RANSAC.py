...
def feature_matching_RANSAC(img1, img2, keypoint_num=None, iter_num=1000, threshold_distance=5):
    ...
    for i in range(iter_num):
        ...
        for idx, match in enumerate(matches):
            ...
            x_prime, y_prime = kp2[trainInd].pt
            xy_matirx = [x, y, 1]
            prime_matrix = [x_prime, y_prime, 1]

            xy_matirx = np.array(xy_matirx)
            prime_matrix = np.array(prime_matrix)

            predict_matrix = np.dot(M, xy_matirx.T)

            distance = L2_distance(prime_matrix, predict_matrix)
            if distance < threshold_distance:
                 count_inlieers += 1
        inliers.append(count_inlieers)


    best_M = M_list[np.array(inliers).argmax()]
...
def main():
    src = cv2.imread('../imgs/library1.jpg')
    src2 = cv2.imread('../imgs/library2.jpg')

    result_RANSAC = feature_matching_RANSAC(src, src2)
    result_LS = feature_matching(src, src2)
    cv2.imshow('input', src)
    cv2.imshow('result RANSAC 20181602', result_RANSAC)
    cv2.imshow('result LS 20181602', result_LS)
